...
if rootclass in baseclasses:
    print ("Error: {} is not a base class. Derives from {}.".format(
        rootclass, ', '.join(baseclasses[rootclass])))
    ok = False
if not ok:
    sys.exit(1)
...
# In the C# code, 0 would mean an invalid index; not found in the class id to index  dictionary
name_to_index = {list(handleclasses)[i-1] : i for i in range(1,len(handleclasses)+1)}
# Can't create this one directly.
del name_to_index["FbxEmitter"]
# pp.pprint(name_to_index)

# name_to_index_func_template = """
# private static System.Collections.Generic.Dictionary<string, int> nameToIndexDict = null;

# private static int GetClassIndex(FbxClassId classId)
# {{
#     var className = classId.GetName();
#     if (nameToIndexDict == null)
#     {{
#         nameToIndexDict = new System.Collections.Generic.Dictionary<string, int>
#         {{
# {}
#         }};
#     }}

#     // return -1 if we can't find the class.
#     return nameToIndexDict.ContainsKey(className) ? nameToIndexDict[className] : -1;
# }}
# """
# name_to_index_entry = """            {{ "{}", {} }},
# """
...
public static FbxObject Realtype (System.IntPtr cPtr, bool ignored)
{
    ...
    // The following code is auto-generated by a script
...
"""
# entries = [name_to_index_entry.format(klass, index) for klass, index in name_to_index.iteritems()]
# name_to_index_function = name_to_index_func_template.format(''.join(entries))

body_statements = [statement_body_template.format(klass, klass) for klass in name_to_index]
constructor_func = statement_start + ''.join(body_statements) + statement_end

# write out the code
with open(output_filename, 'w') as outfile:
    # outfile.write(name_to_index_function)
